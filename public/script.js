...
// Warrant screening function
async function loadWarrant() {
    const resultsDiv = document.getElementById('warrant-results');
    const updateProgress = showLoadingWithProgress('warrant-results', 'Processing warrant screening...');
    ...
    try {
        ...
        const response = await fetch('/api/warrant');
        const data = await response.json();

        updateProgress(100);

        if (data.error) {
            showError('warrant-results', data.error);
            return;
        }

        displayWarrantResults(data);
    } catch (error) {
        showError('warrant-results', 'Failed to load warrant data');
    }
}

function displayWarrantResults(warrants) {
    let html = '';

    if (warrants.length === 0) {
        html = '<div class="error">❌ No warrants meeting criteria found at this time.</div>';
    } else {
        html += `
            <h3>LIST OF ACTIVE WARRANTS</h3>
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
        ...
        warrants.forEach(warrant => {
            html += `
                <tr>
                    ...
                    <td>${formatCurrency(warrant.price)}</td>
                    <td>${formatCurrency(warrant.parentPrice)}</td>
                </tr>
            `;
        });
        ...
        html += `
            ...
            <div class="info">📊 Total: ${warrants.length} active warrants found</div>
        `;
    }
    ...
    html += `
        <div class="warrant-warning">
            ...
            • Warrant trading is highly risky and complex<br>
            • Prices can drop drastically near expiry date<br>
            • This analysis is not a guarantee of profit, do your own research<br>
            • Data may be incomplete due to API limitations<br>
            • Warrant prices may not be real-time due to data source limitations
        </div>

        <div class="warrant-tips">
            <strong>🎯 Tips:</strong><br>
            • Compare warrant prices with their parent stock<br>
            • High trading volume indicates good liquidity
        </div>
    `;

    document.getElementById('warrant-results').innerHTML = html;
}
